<template>
  <div>
    <el-row style="height: 500px;margin: 40px 40px auto 40px">
      <el-tooltip effect="dark" placement="right" v-for="item in items.slice((currentPage - 1) * pageSize, currentPage * pageSize)" :key="item.id">
        <p slot="content" style="font-size: 14px;margin-bottom: 6px;">《{{ item.name }}》</p>
        <p slot="content" style="font-size: 13px;margin-bottom: 6px">
          <span>{{ item.price }}元</span>
        </p>

        <el-card style="width: 135px;margin-bottom: 20px;height: 233px;float: left;margin-right: 15px" class="item" bodyStyle="padding:10px" shadow="hover">
          <router-link
            target="_blank"
            :to="{
              path: '/itemDetail',
          ...
          <div class="info">
            ...
            <i class="el-icon-edit" @click="editItem(item.id)" v-if="showEdit"></i>
          </div>
    ...
    <el-row>
      <el-pagination @current-change="handleCurrentChange" :current-page="currentPage" :page-size="pageSize" :total="items.length"> </el-pagination>
    </el-row>
